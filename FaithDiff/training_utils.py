...
class EMAModel:
    ...
    def save_state_dict(self, path: str) -> None:
        ...
        print(f"EMA model state saved to {path}")
    ...
    @classmethod
    def from_pretrained(cls, path, model_cls, accelerator, args) -> "EMAModel":
        _, ema_kwargs = model_cls.load_config(path, return_unused_kwargs=True)
        model = model_cls.from_pretrained(path)

        ema_model = cls(
            args=args, accelerator=accelerator, parameters=model.parameters(), model_cls=model_cls,
    ...
    def pin_memory(self) -> None:
        ...
        if torch.backends.mps.is_available():
            print("Apple silicon does not support pinned memory. Skipping.")
            return
