...
class SCBNet(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.controlnet_cond_embedding = nn.Conv2d(in_channels, block_out_channels[0], kernel_size=conv_in_kernel,
    ...
    def forward(
        ...
        # 2. pre-process
        sample = self.controlnet_cond_embedding(cond_img)
...
def main():
    # 1. 加载预训练 UNet（你可以替换为自己的路径）
    print("Loading UNet...")
    backup_unet = UNet2DConditionModel.from_pretrained("/data/czh/models/RealVisXL_V4.0", subfolder="unet")
    # -------
    ...
    # -------
    scb_net = SCBNet.from_unet(backup_unet, load_weights_from_unet=True)

    # 3. 计算并输出训练参数量
    total_params = count_trainable_params(scb_net)
    print(f"\n✅ SCBNet Trainable Parameters: {total_params:,} ({total_params * 4 / (1024 ** 2):.2f} MB)")
