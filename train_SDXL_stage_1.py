...
def log_validation(unet, args, accelerator, weight_dtype, step):
    ...
    vae = AutoencoderKL.from_pretrained(args.pretrained_vae_model_name_or_path, subfolder="vae").to(
        dtype=TORCH_DTYPE_MAPPING[args.vae_precision])
    ...
    test_img_list = os.listdir('/data/czh/data/train/allinone/low-light/LQ/')[:10]
    img_count = 0
    images = []
    for file_name in test_img_list:

        data = json.load(open(os.path.join('./json/low-light', file_name.split('.')[0] + '.json')))
        lr_image = Image.open(os.path.join('/data/czh/data/train/allinone/low-light/LQ', file_name)).convert('RGB')
        ...
        gen_image = pipe(lr_img=lr_image, prompt=prompt_init, negative_prompt=negative_prompt_init,
                         num_images_per_prompt=num_samples, start_point='noise', num_inference_steps=20, seed=42,
    ...
    out_img.save(os.path.join(args.output_dir, 'iter_' + str(step) + '.png'))
...
def parse_args():
    ...
    parser.add_argument(
        "--validation_caption_dir", type=str, default=None, help="validation_set."
    )
    parser.add_argument(
        "--validation_steps",
        type=int,
...
def main():
    ...
    for param in unet.information_transformer_layers.parameters():
        param.requires_grad = True
    ...
    if args.use_ema:
        ema_unet = EMAModel(args, accelerator, unet.parameters(), model_cls=UNet2DConditionModel,
    ...
    # optimizer
    params_to_opt = itertools.chain(unet.condition_embedding.parameters(),
                                    unet.information_transformer_layers.parameters(),
                                    unet.spatial_ch_projs.parameters())

    optimizer_grouped_parameters = [
        {'params': params_to_opt, 'lr': args.learning_rate, 'weight_decay': args.adam_weight_decay,
         'betas': (args.adam_beta1, args.adam_beta2), 'eps': args.adam_epsilon}]
    optimizer = torch.optim.AdamW(optimizer_grouped_parameters)

    def collate_fn(data):
        # valid_data = [example for example in data if example["image"].size(1) > 0 and example["image"].size(2) > 0]
        # if len(valid_data) == 0:
        #     raise ValueError("All images in this batch are invalid.")
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("FaithDiff_stage_1", config=vars(args))
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                with torch.no_grad():
                    ...
                    hq_latents = vae.encode(
                        batch["images"].to(accelerator.device, dtype=vae_precision)).latent_dist.sample()
            ...
            logs = {"noise_step_loss": diffusion_loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
